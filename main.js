...
// ----- Globális változók -----
let transactions = [];
// Alapértelmezett kategóriák, melyek beszúródnak, ha üres a DB
let defaultCategories = [
  { name: "Egyéb", color: "#000000", icon: "❓" },
  { name: "Étel", color: "#FF5722", icon: "🍔" },
  { name: "Utazás", color: "#3F51B5", icon: "✈️" }
];
let categories = []; // Ide a DB-ből töltjük be
let monthlyBudget = 0;
let currentMonthExpenses = 0;
let localSavingsBalance = 0;
let chart; // Chart.js diagram
...
// ----- IndexedDB-műveletek -----
// Alapértelmezett értékek beszúrása, ha a táblák üresek
...
async function loadAppData() {
  ...
  let cats = await getCategoriesFromDB();
  if (cats.length === 0) {
...
function editCategory(cat) {
  ...
  if (newName && newName.trim() !== "") {
    ...
    // Egyszerűen csak frissítjük az objektumot az UI-ban,
    // majd manuálisan cseréljük ki a kategóriák tömbjében
    const updatedCat = { name: newName, color: newColor || cat.color, icon: newIcon || cat.icon };
    // Töröljük a régi kategóriát, majd hozzáadjuk az újat
    deleteCategory(cat.name).then(() => {
      categories.push(updatedCat);
      // Mivel IndexedDB-ben a primary key a név, így újra beszúrjuk:
      const tx = db.transaction("categories", "readwrite");
      const store = tx.objectStore("categories");
      store.put(updatedCat).onsuccess = () => {
        loadAppData();
      };
    });
  }
...
function renderTransactions(transList) {
  ...
  transList.forEach(tx => {
    ...
    delBtn.addEventListener("click", () => {
      deleteTransactionFromDB(tx.id).then(() => {
        loadAppData();
      });
    });
...
// Chart.js kördiagram frissítése
...
transactionForm.addEventListener("submit", async (event) => {
  ...
  try {
    const insertedTx = await addTransaction(newTx);
    transactions.unshift(insertedTx);
    renderTransactions(transactions);
    updateMonthlyExpenses();
    updateChart();
